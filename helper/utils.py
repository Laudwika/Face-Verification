...
def get_feature_bank(subject = 's36'):
    ## FEATURE LOADER / NPY ##
    subject_path = config.face_bank_loc + subject + '.npy'
    ...
    rf = random.randint(0, len(out))
    return out[rf]

#Compute scores
def evaluation(featureL, featureR, mu):
...
def verify_result(scores, threshold):
    if scores > threshold:
        result = 'SAME'
    else:
        result = 'DIFFERENT'
